...
class UploadHandler
{
    ...
    protected function get_file_object($fileName)
    {
        ...
        if ($this->is_valid_file_object($fileName)) {
            ...
            $qb = $this->entityManager->getRepository($this->options['entity_image_path'])
            ...
            if(!$image instanceof \CoreBundle\Entity\Image && !$image instanceof \BlogBundle\Entity\Image){
                return null;
            }
            
            
            $file = new \stdClass();
            $file->title = $image->getTitle();
            $file->alt = $image->getAlt();
            $file->name = $fileName;
    ...
    protected function get_file_objects($iterationMethod = 'get_file_object')
    {
        ...
        $returnValues = array_values(array_filter(array_map(
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                if(isset($upload['type'][0]) && isset($upload['name'][0])){
                    ...
                    if($extension=='image/jpeg') {
                        ...
                        if(preg_match('/\.jpeg/', $files[$index]->name)) $arr = explode('.jpeg', $files[$index]->name);
                        if(preg_match('/\.jpg/', $files[$index]->name)) $arr = explode('.jpg', $files[$index]->name);
                        $img_name = $arr[0];
                    }else if($extension=='image/gif') {
                        $source = imagecreatefromgif($this->options['upload_dir'].$files[$index]->name);
                        if(preg_match('/\.gif/', $files[$index]->name)) $arr = explode('.gif', $files[$index]->name);
                        $img_name = $arr[0];
